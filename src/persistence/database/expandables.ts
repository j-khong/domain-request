...
import { DomainExpandables, DomainWithExpandablesRequest } from '../../DomainRequest';
import {
   DbRecord,
...
> extends SimpleDatabaseTable<DRN, F, TF> {
   ...
   protected createResultToPopulate(dbRecord: DbRecord, allFieldsToSelect: FieldsToSelect<F>): any {
      ...
      this.populateWithOneToOneExpandables(this.tableConfig, result, this.expandablesFieldsToSelectTmpToDel, dbRecord);

      return result;
   }

   private populateWithOneToOneExpandables<F, E, TF extends string>(
   ...
   private async fetchExpandablesOneToMany(
      ...
      for (const expKey in expandables) {
         ...
         for (const result of res.results) {
            ...
            if (toPopulate === undefined) {
               console.log(
                  `big problem, cannot find resource ${tableConfig.tableName} of id [${resourceId as string}]`,
               );
               continue;
            }
